...
- **Overview** shows deployment identity, readiness issues, declared customizations, Flux drift results, and scan coverage for observed container digests.
- **Packages** compares Standard, Configured, and Deployed state. Opening a package compares enablement, chart sources, images, replicas, resources, storage, and ingress. Installed child-chart values are read at the observed Helm revision. Sensitive paths are omitted and URL credentials are stripped.
...
A declared customization is a difference from the selected baseline. Runtime drift is reported only when Flux supplies a current `Drifted` condition with drift detection enabled; older controllers or missing conditions show **Not checked**. Flux exclusions still apply. This is not an exhaustive comparison of every Kubernetes object or every chart value. Complex Helm `valuesFrom.targetPath` expressions are marked incomplete rather than guessed. Missing APIs, permissions, or values also remain explicitly unknown.
...
The shell follows the Astronomer reference used by the neighboring admin UI: Inter body/navigation, JetBrains Mono code, a 16px base, 14px navigation, 240px expanded / 64px collapsed sidebar, and a 56px desktop header. Fonts are self-hosted with their licenses. Genesis keeps its own brand and theme colors. A vector favicon, early theme initialization, theme-aware browser color, safe-area insets, keyboard search/focus, and responsive navigation are included. This is a source-based geometry match, not a pixel-diff certification against a supplied design screenshot.
