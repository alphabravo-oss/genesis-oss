...
Set `cluster.namespace`, `cluster.release`, and `cluster.releaseNamespaces` to the actual umbrella and child Helm storage namespaces (`status.storageNamespace`, then `spec.storageNamespace`, defaulting to the HelmRelease namespace). Workload target namespaces can differ. All listed namespaces must already exist. The chart grants pod and route inventory reads cluster-wide; Secret/ConfigMap reads are limited to the listed namespaces, because Helm stores release values there. Those credentials can read other secrets in those namespaces too; use a dedicated service account. There are no cluster write grants. An in-cluster install uses its service account automatically and needs no mounted kubeconfig. Publish through a TLS reverse proxy if remote access is required; the supplied Service remains ClusterIP.
...
Catalog pages load independently of cluster status. Filters stay in the URL and run locally; tables show 50 rows per page, and CSV exports every filtered row. Package settings load on their dedicated detail page. Cluster reads run concurrently with a 10-second deadline and a 30-second cache. A failed refresh retains the last observation with its timestamp and a warning.
...
The header identifies the observed cluster and installed Genesis version. Packages owns the full comparison release, profile controls, and cross-version notice; Overview shows a compact comparison summary. Images exposes catalog selection in catalog tabs and an image's catalog comparison; Scans exposes it beside bulk catalog actions. Services shows live cluster routes without comparison controls.

The comparison defaults to the installed release and recorded installation profiles. **Compare against** selects another release explicitly; both release versions remain visible. If the installed release cannot be confirmed or has no local catalog, automatic comparison stays unavailable until an explicit release is selected. It never presents a different release as the installed reference.

Package summaries describe confirmed chart-version, enablement, and configuration differences using neutral badges. Package details repeat the difference summary. Each setting explains whether the cluster differs from the comparison release, the installed value differs from cluster configuration, or both. Changed settings have a slim row marker and changed value cells have a distinct comparison highlight; unavailable comparisons stay explicit. The three values remain side by side without a redundant status column. Differences are informational; health problems and Flux runtime drift remain separate. All packages are inspected regardless of profile selection. Profile checkboxes add optional configuration to the comparison release; they are not package filters and should not all be selected by default. Installations without a usable profile record compare release defaults automatically; applied profile filenames and order cannot be recovered reliably from running packages. Profile options are an advanced override: normally use recorded installation profiles; select profiles manually only when the installation record is missing or for a deliberate alternative comparison. A profile changes the expected configuration, never the cluster.

- **Overview** shows deployment identity, readiness issues, differences from the comparison release, Flux drift results, and scan coverage for observed container digests.
- **Packages** separates Comparison release, Cluster configuration, Installed chart, Comparison result, and Flux runtime drift. Clicking a package row opens `/packages/<name>` to compare enablement, chart sources, images, replicas, resources, storage, and ingress. The back link preserves list filters and comparison choices; legacy `?pkg=` links redirect to the detail page. Installed child-chart values are read at the observed Helm revision from the active Flux storage namespace; confirmed installed umbrella values are also shown. Failed reads name the release storage location and revision. Sensitive paths are omitted and URL credentials are stripped.
...
Comparison controls appear where they affect the view: the comparison release and ordered profiles on Packages, a compact summary on Overview, and catalog versions alongside image comparisons and bulk scans. Services has no comparison panel. Cross-version comparisons explain that release changes can explain differences. Profiles change only the package comparison release, never the image catalog or cluster. Package and image rows open their own detail pages; checkbox selection remains independent of navigation. Packages also links to selected-release notes, source changes between the installed and selected versions, and the upgrade guide. These sources do not constitute automatic migration-impact analysis.
