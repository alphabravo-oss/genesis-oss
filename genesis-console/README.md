...
The header identifies the observed cluster and installed Genesis version. Packages owns the full comparison standard, profile controls, and cross-version notice; Overview shows a compact comparison summary. Images exposes catalog selection in catalog tabs and an image's catalog comparison; Scans exposes it beside bulk catalog actions. Services shows live cluster routes without comparison controls.

Package and setting rows with confirmed differences have an amber background and edge marker, alongside their written comparison status. Matching and unknown rows remain unhighlighted. Profile options are an advanced override: normally use recorded installation profiles; select profiles manually only when the installation record is missing or for a deliberate alternative comparison. A profile changes the expected configuration, never the cluster.
...
Comparison controls appear where they affect the view: the full standard and ordered profiles on Packages, a compact summary on Overview, and catalog versions alongside image comparisons and bulk scans. Services has no comparison panel. Cross-version comparisons warn that release changes can explain differences. Profiles change only the package comparison standard, never the image catalog or cluster. Package and image rows open their own detail pages; checkbox selection remains independent of navigation. Packages also links to selected-release notes, source changes between the installed and selected versions, and the upgrade guide. These sources do not constitute automatic migration-impact analysis.
