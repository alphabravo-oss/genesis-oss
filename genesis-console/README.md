...
Read-only deployment inspection and comparison for Genesis.
...
Without cluster credentials, catalogs and public-image scans work and deployment observations show unavailable. To connect a cluster, open **Cluster connection** (user menu or the header), name the connection, and paste or choose a kubeconfig. **Test connection** shows the API server, Kubernetes version, and Genesis release before **Save and connect**. The kubeconfig is stored encrypted with `.local/connection-key` (generated by `up.sh`; keep it with your backups, separately from the database volume) and is never shown again. For a cluster on this machine whose API is `https://127.0.0.1:PORT` or `https://0.0.0.0:PORT`, tick the `host.docker.internal` option; the certificate name is kept, so TLS verification still works. The console checks the kubeconfig: login plugins and certificate file paths are rejected with instructions.

A kubeconfig can still be supplied at startup instead: `KUBECONFIG=/absolute/path/to/config python3 scripts/kubeconfig.py`, then `GENESIS_KUBECONFIG="$PWD/.local/kubeconfig" ./scripts/up.sh` (add `--server https://host.docker.internal:PORT` to the helper for a local cluster). A connection saved in the UI takes precedence.

### Service account for an external console

For EKS, GKE, AKS, or any cluster whose users sign in through a plugin, create the console's read-only service account in that cluster and paste its kubeconfig:

```bash
helm install genesis-console-reader deploy/chart -n genesis-console --create-namespace --set console.enabled=false
./scripts/service-account-kubeconfig.sh genesis-console-reader genesis-console
```

The service account has the same read-only permissions as the in-cluster install. The API server must be reachable from wherever the console runs.
...
`deploy/chart` installs the console, Service, cache PVC, probes, and read-only service account. Use an existing PostgreSQL database and an existing Secret named by `credentialsSecret`, with keys `database-url` and `console-password`. Add an optional `connection-key` (`openssl rand -base64 32`) to save cluster connections from the UI. A secret can be created from local files without exposing values in command arguments:
...
Package summaries describe confirmed chart-version, enablement, and configuration differences using neutral badges. Package details repeat the difference summary. Each setting explains whether the cluster differs from the comparison release, the installed value differs from cluster configuration, or both. Changed settings have a slim row marker and changed value cells have a distinct comparison highlight; unavailable comparisons stay explicit. The three values remain side by side without a redundant status column. Differences are informational; health problems and Flux runtime drift remain separate. Equivalent values are not differences: null equals unset, and Flux chart defaults (`version: "*"`, `reconcileStrategy: ChartVersion`) equal an unset baseline. A package Git source such as an air-gap mirror (`git.repo`) is shown as installation-specific and not counted. All packages are inspected regardless of profile selection. Profile checkboxes add optional configuration to the comparison release; they are not package filters and should not all be selected by default. Installations without a usable profile record compare release defaults automatically; applied profile filenames and order cannot be recovered reliably from running packages. Profiles are optional and default to Automatic: normally use recorded installation profiles; select profiles manually only when the installation record is missing or for a deliberate alternative comparison. A profile changes the expected configuration, never the cluster.

- **Overview** opens with a one-line status, then cards for package health (ready of installed), Flux drift, images with Critical or High findings (with scan coverage), and informational differences from the comparison release. **Needs attention** lists unreadable data sources, unhealthy or drifted packages, the most severe images, and unscanned images. A side panel shows the cluster, comparison, installation record, and data-source status.
- **Packages** opens with a one-line comparison summary and three tabs: the package table, **Comparison settings** (release, Automatic or chosen profiles, how the comparison works), and **Upgrade guidance**. The table separates Comparison release, Cluster configuration, Installed chart, Comparison result, and Flux runtime drift. Clicking a package row opens `/packages/<name>` to compare enablement, chart sources, images, replicas, resources, storage, and ingress. The back link preserves list filters and comparison choices; legacy `?pkg=` links redirect to the detail page. Installed child-chart values are read at the observed Helm revision from the active Flux storage namespace; confirmed installed umbrella values are also shown. Failed reads name the release storage location and revision. Sensitive paths are omitted and URL credentials are stripped.
- **Images** starts on **Running in cluster**: unique deployed images, grouped by observed digest (or configured reference when unavailable), including custom images. Open an image to see its own detail page with findings, references, digest, pods, and containers. The back link preserves the release and list filters. Select individual images or all filtered images, then **Scan N selected**. Scans stay on this page: row status and finding counts update automatically. The image detail page uses the same live updates and also supports scanning that image. Deployed scans recheck the pod inventory, pin observed digests when available, and scan each digest only once regardless of replica count. **Release catalog** has filters for default-on, unscanned, critical, and no-public-source images, and uses the same inline scan workflow. Images whose older scans collected only High and Critical get one banner with a rescan action. Saved findings are matched to scanned digests, including images outside the catalog; unscanned images stay explicitly unscanned.
...
| `GENESIS_CONNECTION_KEY_FILE` / `GENESIS_CONNECTION_KEY` | 32-byte base64 key that encrypts saved cluster connections; file takes precedence. Unset disables saving |
| `GENESIS_CONNECTION_DIR` | Where the decrypted saved kubeconfig is written (a private per-process directory inside it); the temp directory by default, a memory-backed volume in the chart |
