...
- Addition of values for `networkPolicies`, `istio`, and `bbtests`
...
1. Navigate to the [Prometheus targets page](https://prometheus.dev.bigbang.mil/targets) and validate that the Kyverno Reporter servicemonitor shows up and is healthy.
1. Navigate to Grafana and search for [policy dashboards](https://grafana.dev.bigbang.mil/?orgId=1&search=open&query=policy). Validate that 3 dashboards appear in the search and data is loaded in each. It can be helpful to deploy an additional package at this point to cause some additional policy violations/reports.
