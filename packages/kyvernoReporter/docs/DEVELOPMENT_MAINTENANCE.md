...
1. Navigate to Grafana and search for [policy dashboards](https://grafana.dev.bigbang.mil/dashboards?query=policy). Validate that 3 dashboards appear in the search and data is loaded in each. It can be helpful to deploy an additional package at this point to cause some additional policy violations/reports.
