...
1. If the big-bang/base image has been updated, the "updater" functionality should be verified - this updates the CVE database by forcing a pull of the `scanner` image. Prefer the dedicated Helm test:

```bash
helm test neuvector -n neuvector --logs --filter name=neuvector-updater-pod-helm-test
```

This test restarts the scanner deployment, so it requires both `bbtests.enabled=true` and `bbtests.updater.enabled=true`.

If creating a manual job instead, preserve the `neuvector-updater-pod-*` name prefix. Big Bang's Kyverno hardening uses that prefix to allow token automount for updater pods; other names can fail with a false-negative `401 Unauthorized`.

```bash
kubectl create job --from=cronjob/neuvector-updater-pod -n neuvector neuvector-updater-pod-manual
```
