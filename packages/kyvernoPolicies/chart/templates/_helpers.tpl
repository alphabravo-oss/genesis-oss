...
{{/* ---- BB CEL-era policy helpers (celPoliciesBeta) ----
     Three naming prefixes:
       cel-*  — shared across VPol and MPol (may extend to GPol later).
                These emit spec fields common to policies.kyverno.io/v1 kinds.
       vpol-* — ValidatingPolicy-specific (validationActions, allContainers, vpol-failurePolicy).
                Stay vpol- until another kind proves it needs them.
       mpol-* — MutatingPolicy-specific (mpol-failurePolicy). GPol has no failurePolicy field. */}}
...
{{- $globalBeta := default "" .Values.celPoliciesBeta.vpolFailurePolicy -}}
...
{{/* MPol-only failurePolicy. Separate from vpol-failurePolicy because MPol has no
     validationFailureAction to auto-derive from. Reads its own global key
     (mpolFailurePolicy) to keep the behavioral difference explicit during migration.
     GPol has no failurePolicy field at all (confirmed via CRD).
     Precedence: per-policy → celPoliciesBeta.mpolFailurePolicy → Values.failurePolicy → Fail.
     Tests: vpol-shared-helpers_test.yaml */}}
{{- define "bb-kyverno-policies.mpol-failurePolicy" -}}
{{- $name := .name -}}
{{- $perPolicy := dig $name "failurePolicy" "" .Values.celPoliciesBeta -}}
{{- $globalBeta := default "" .Values.celPoliciesBeta.mpolFailurePolicy -}}
{{- $fallback := default "Fail" .Values.failurePolicy -}}
{{- default (default $fallback $globalBeta) $perPolicy -}}
{{- end -}}
...
{{/* No upstream helper — upstream VPol template omits background entirely (defaults to Kyverno's
     ...
     Changes: wires to evaluation.background.enabled with per-policy and celPoliciesBeta
     global overrides. Uses toString to distinguish false (explicit) from "" (not set).
     Shared across VPol/MPol — all three kinds have evaluation.background.
     Precedence: per-policy → celPoliciesBeta global → Values.background.
     Tests: vpol-shared-helpers_test.yaml */}}
{{- define "bb-kyverno-policies.cel-background" -}}
{{- $name := .name -}}
...
{{/* No upstream equivalent — upstream CEL templates do not set webhookConfiguration.timeoutSeconds.
     BB CPol uses the kyverno-policies.webhookTimeoutSeconds helper for the same purpose.
     Shared across VPol/MPol — all three kinds have spec.webhookConfiguration.
     Precedence: per-policy → celPoliciesBeta global → Values.webhookTimeoutSeconds.
     Tests: vpol-shared-helpers_test.yaml */}}
{{- define "bb-kyverno-policies.cel-webhookTimeoutSeconds" -}}
{{- $name := .name -}}
...
{{/* No upstream equivalent — upstream CPol uses the pod-policies.kyverno.io/autogen-controllers
     annotation; upstream CEL templates omit autogen entirely (Kyverno defaults to all controllers).
     Converts CamelCase CSV format (.Values.autogenControllers) to the spec.autogen.podControllers
     list (lowercase plural per Kyverno source pkg/cel/autogen/data.go). Accepts same input format
     as CPol so users don't learn a new syntax. "none" suppresses the autogen block entirely.
     Shared across VPol/MPol — all three kinds have spec.autogen.
     Precedence: per-policy → celPoliciesBeta global → Values.autogenControllers.
     Tests: vpol-shared-helpers_test.yaml */}}
{{- define "bb-kyverno-policies.cel-autogenControllers" -}}
{{- $name := .name -}}
...
{{/* Namespace exclusion via matchConditions CEL.
     ...
     resources.names, subjects, clusterRoles, roles). CEL matchConstraints has no equivalent
     ...
     Shared across VPol/MPol/GPol — all three kinds use matchConditions for namespace exclusion.
...
- name: exclude-namespaces
  expression: {{ printf "!(%s in %s)" $field ($merged | toJson) | quote }}
{{- end }}
{{- end -}}

{{/* Preserve the original helper name for namespaced-resource callers.
     Namespace-targeting policies opt into cel-excludeNamespacesBy with object.metadata.name. */}}
{{- define "bb-kyverno-policies.cel-excludeNamespaces" -}}
{{- include "bb-kyverno-policies.cel-excludeNamespacesBy" . -}}
{{- end -}}
