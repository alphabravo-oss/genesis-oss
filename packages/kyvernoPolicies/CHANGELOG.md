...
---
## [3.3.4-bb.87] (2026-07-08)

### Fixed

- Update public key used by Ironbank image signature verification policy

## [3.3.4-bb.86] (2026-07-01)

### Added

- CEL-based MutatingPolicy: `update-image-registry-cel`, gated behind `celPoliciesBeta.update-image-registry-cel.enabled` (disabled by default). Rewrites the image registry on containers, initContainers, and ephemeralContainers per `parameters.update`, a list of `{from, to}` rewrites where `from` is an RE2 regex matched against the image and the matched text is replaced with the literal `to` (later entries see earlier rewrites, so they chain). Mirrors the legacy `update-image-registry` ClusterPolicy. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.85] (2026-06-17)

### Fixed

- `celPoliciesBeta.excludeNamespaces` rejected all pod-controller admissions under autogen; the namespace matchCondition now compares `request.namespace` instead of `object.metadata.namespace`. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).
- `update-automountserviceaccounttokens-cel` did not match controller-created pods, whose `pods.allow`/`pods.deny` names are only present via `generateName` at admission; the pod scope now falls back from `name` to `generateName`. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).
- `disallow-privilege-escalation-cel` denied pods that left `allowPrivilegeEscalation` undefined; matching the legacy policy, undefined and `false` now pass and only an explicit `true` is denied. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.84] (2026-06-16)

### Fixed

- `additionalPolicyExceptions` could only emit legacy `kyverno.io/v2` PolicyExceptions, leaving no way to except a CEL-based policy. Entries now accept optional `apiVersion` (default `kyverno.io/v2`) and `kind` (default `PolicyException`) overrides plus an `enabled` flag (default `true`); set `apiVersion: policies.kyverno.io/v1` with a `spec` of `policyRefs` + `matchConditions` to except a ValidatingPolicy/MutatingPolicy/GeneratingPolicy. Legacy behavior is unchanged when omitted. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.83] (2026-06-16)

### Added

- `update-automountserviceaccounttokens-cel` now emits a `policies.kyverno.io/v1` PolicyException (`<name>-exception`) exempting the `pods.allow` workloads from the `disallow-auto-mount-service-account-token-cel` ValidatingPolicy, which would otherwise reject them once they are mutated to `automountServiceAccountToken: true`. Mirrors the legacy ClusterPolicy's bundled PolicyException. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.82] (2026-06-08)

### Added

- Updated gluon scripts to run the chainsaw tests in parallel and, for the vpol and mpol suites, with `--fast-namespace-deletion` so they no longer block on test namespace finalization.

## [3.3.4-bb.81] (2026-06-04)

### Added

- CEL-based MutatingPolicy: `update-automountserviceaccounttokens-default-cel`, gated behind `celPoliciesBeta.update-automountserviceaccounttokens-default-cel.enabled` (disabled by default). Disables automounting of the API token on the `default` ServiceAccount in each namespace listed in `namespaces`. The single values input renders two MutatingPolicies: an admission policy (`update-automountserviceaccounttokens-default-cel`) that mutates each `default` ServiceAccount synchronously as it is created, and a mutate-existing policy (`update-automountserviceaccounttokens-default-cel-existing`) that retrofits `default` ServiceAccounts that already exist. Mirrors the legacy `update-automountserviceaccounttokens-default` ClusterPolicy. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.80] (2026-06-03)

### Added

- CEL-based ValidatingPolicy: `restrict-user-id-cel`, gated behind `celPoliciesBeta.restrict-user-id-cel.enabled` (disabled by default). Validates that pod-level and per-container `runAsUser` values are undefined or fall in an allowed range. `parameters.allow` uses the structured CEL range shape from `restrict-group-id-cel` (`{min, max}`, both inclusive, either optional) instead of the legacy CPol string expression shape. Defaults to `allow: [{min: 1000}]`. Mirrors the legacy `restrict-user-id` ClusterPolicy behavior. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.79] (2026-06-02)

### Added

- CEL-based MutatingPolicy: `update-image-pull-policy-cel`, gated behind `celPoliciesBeta.update-image-pull-policy-cel.enabled` (disabled by default). Sets `imagePullPolicy` on tagged (non-digest-pinned) containers, initContainers, and ephemeralContainers per `parameters.update`, a list of `{from, to}` rewrites (an empty `from` matches any current value; `to` must be `Always`, `Never`, or `IfNotPresent`; later entries take precedence). Mirrors the legacy `update-image-pull-policy` ClusterPolicy. Part of [Epic 578](https://repo1.dso.mil/groups/big-bang/-/epics/578).

### Fixed

- The kyverno cli tests for the other MutatingPolicies were not properly validating mutations. This was fixed using `patchedResources`.
- `add-default-capability-drop-cel` now mutates controller pod templates (Deployment, ReplicaSet, DaemonSet, StatefulSet) in addition to bare Pods, restoring parity with the legacy ClusterPolicy. Because it mutates an atomic list (`capabilities.drop`) it must use a JSONPatch mutation, which Kyverno autogen cannot rewrite for controllers, so the policy now matches controllers directly and uses a kind-aware CEL expression targeting `spec.template.spec`. This also stops the policy from denying controller admission on Kyverno >= 1.18, where the autogen-generated controller mutation referenced a `spec.containers` path that does not exist on controllers.

## [3.3.4-bb.78] (2026-05-29)

### Added

- CEL-based ValidatingPolicy: `restrict-host-ports-cel`, gated behind `celPoliciesBeta.restrict-host-ports-cel.enabled` (disabled by default). Validates that every `hostPort` defined on containers, initContainers, and ephemeralContainers falls within an allowed range. `parameters.allow` takes a list of `{min, max}` entries (both inclusive, either optional); an empty list denies all host ports. Mirrors the legacy `restrict-host-ports` ClusterPolicy. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.77] (2026-05-28)

### Added

- CEL-based ValidatingPolicy: `restrict-host-path-write-cel`, gated behind `celPoliciesBeta.restrict-host-path-write-cel.enabled` (disabled by default). For every volume with `hostPath`, requires every `volumeMount` referencing it across containers, initContainers, and ephemeralContainers to set `readOnly: true`, unless the `hostPath.path` matches an entry in `parameters.allow` as an auto-anchored regex pattern. Empty `allow` requires every hostPath mount to be readOnly. Mirrors the legacy `restrict-host-path-write` ClusterPolicy. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.76] (2026-05-27)

### Added

- CEL-based ValidatingPolicy: `restrict-selinux-type-cel`, gated behind `celPoliciesBeta.restrict-selinux-type-cel.enabled` (disabled by default). Validates that `seLinuxOptions.type` on pods, ephemeral containers, init containers, and regular containers is undefined or listed in `parameters.allow`. Mirrors the legacy `restrict-selinux-type` ClusterPolicy. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.75] (2026-05-27)

### Added

- CEL-based ValidatingPolicy: `restrict-host-path-mount-cel`, gated behind `celPoliciesBeta.restrict-host-path-mount-cel.enabled` (disabled by default). Validates that every `spec.volumes[*].hostPath.path` matches an entry in `parameters.allow` as an auto-anchored regex pattern. Mirrors the legacy `restrict-host-path-mount` ClusterPolicy. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.74] (2026-05-26)

### Added

- CEL-based ValidatingPolicy: `restrict-host-path-mount-pv-cel`, gated behind `celPoliciesBeta.restrict-host-path-mount-pv-cel.enabled` (disabled by default). Validates that PersistentVolume `spec.hostPath.path` is on the allowed list while allowing non-hostPath PersistentVolumes. The allow list preserves the legacy path values shape, including `*` wildcards. Mirrors the legacy `restrict-host-path-mount-pv` ClusterPolicy. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.73] (2026-05-19)

### Added

- CEL-based ValidatingPolicy: `restrict-group-id-cel`, gated behind `celPoliciesBeta.restrict-group-id-cel.enabled` (disabled by default). Validates that pod-level `runAsGroup`, per-container `runAsGroup` (containers, initContainers, ephemeralContainers), `fsGroup`, and each `supplementalGroups[*]` ID falls in an allowed range and not in a denied range. Allow and deny each take a list of `{min, max}` entries (both inclusive, either optional). Defaults to `allow: [{min: 1000}]`. Mirrors the legacy `restrict-group-id` ClusterPolicy. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.72] (2026-05-18)

### Added

- CEL-based ValidatingPolicy: `restrict-capabilities-cel`, gated behind `celPoliciesBeta.restrict-capabilities-cel.enabled` (disabled by default). Validates that `securityContext.capabilities.add` on ephemeral, init, and regular containers only contains capabilities listed in `parameters.allow` (defaults to `[NET_BIND_SERVICE]`; empty list denies all additions). Mirrors the legacy `restrict-capabilities` ClusterPolicy. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.71] (2026-05-15)

### Added

- CEL-based ValidatingPolicy: `require-requests-equal-limits-cel`, gated behind `celPoliciesBeta.require-requests-equal-limits-cel.enabled` (disabled by default). Validates that every container has both `resources.requests.cpu`/`memory` and `resources.limits.cpu`/`memory` set and that requests equal limits (so pods land in the Guaranteed QoS class). Mirrors the legacy `require-requests-equal-limits` ClusterPolicy. Part of [Epic 578](https://repo1.dso.mil/groups/big-bang/-/epics/578).

## [3.3.4-bb.70] (2026-05-15)

### Added

- CEL-based ValidatingPolicy: `restrict-apparmor-cel`, gated behind `celPoliciesBeta.restrict-apparmor-cel.enabled` (disabled by default). Validates legacy per-container AppArmor annotations against `parameters.allow`, matching the legacy `restrict-apparmor` ClusterPolicy behavior. Part of [Epic 661](https://repo1.dso.mil/groups/big-bang/-/epics/661).

## [3.3.4-bb.69] (2026-05-14)

### Added

- CEL-based `ImageValidatingPolicy`: `require-image-signature-cel`, gated behind `celPoliciesBeta.require-image-signature-cel.enabled` (disabled by default). Extends the CI tests with IVPol discovery (`06_test-cel-chainsaw.sh`) and a parallel `quiet_ivpols`/`restore_ivpols` cleanup pair in `_helpers.sh`. Mirrors the legacy `require-image-signature` ClusterPolicy. Part of [Epic 578](https://repo1.dso.mil/groups/big-bang/-/epics/578).

## [3.3.4-bb.68] (2026-05-13)

### Added

- CEL-based ValidatingPolicies: `disallow-tolerations-cel` and `disallow-tolerations-cel-runtimeclass`, both gated behind `celPoliciesBeta.disallow-tolerations-cel.enabled` (disabled by default). Both VPols read the same `parameters.disallow` list. Each entry can set `key`, `value`, and/or `effect` as auto-anchored RE2 regex; a toleration is blocked when every present field matches. Mirrors the legacy `disallow-tolerations` ClusterPolicy. Part of [Epic 578](https://repo1.dso.mil/groups/big-bang/-/epics/578).

## [3.3.4-bb.67] (2026-05-12)

### Added

- CEL-based ValidatingPolicy: `disallow-rbac-on-default-serviceaccounts-cel`, gated behind `celPoliciesBeta.disallow-rbac-on-default-serviceaccounts-cel.enabled` (disabled by default). Denies RoleBinding and ClusterRoleBinding subjects that grant access to the `default` ServiceAccount or Kubernetes service account groups. The default `matchConditions` preserve the legacy exclusion for `system:service-account-issuer-discovery` and can be reset with values when a cluster needs to enforce that path. Part of [Epic 578](https://repo1.dso.mil/groups/big-bang/-/epics/578).

## [3.3.4-bb.66] (2026-05-11)

### Added

- CEL-based ValidatingPolicy: `disallow-pod-exec-cel`, gated behind `celPoliciesBeta.disallow-pod-exec-cel.enabled` (disabled by default). Denies `kubectl exec` and `kubectl attach` against pods by intercepting the `pods/exec` and `pods/attach` CONNECT admission requests. Mirrors the legacy `disallow-pod-exec` ClusterPolicy. Also adds `pods/exec` and `pods/attach` to the bbtest ClusterRole so the test pod's ServiceAccount can drive the chainsaw test, and simplifies `tests/scripts/vpol-chainsaw-test.sh` to derive the policy name from the test directory's basename instead of reading `kyverno-test/kyverno-test.yaml`. Part of [Epic 578](https://repo1.dso.mil/groups/big-bang/-/epics/578).
