...
Profiles live in `profiles/`. The default install does not use them. Select a profile at install or upgrade time:

```bash
python3 scripts/install.py --profile keycloak -f /private/path/keycloak-values.yaml
```

| File | What it adds |
|---|---|
| `keycloak.yaml` | Keycloak `quay.io/keycloak/keycloak:26.7.2`. The bundled database chart expects a Bitnami layout. No public `postgresql:18.4` image of that layout was found, so the profile turns that chart off. Point `addons.keycloak.database.host` at Postgres 18. `docker.io/library/postgres:18.4` was verified. |
...
Repeat `--profile` in application order; custom `-f` files apply after all profiles. The installer records profile names and checksums for automatic console comparison. Keycloak plus `--profile ldap` is the single sign-on shape: Keycloak is the broker, and the directory is one you already operate. Add-on settings use `addons.<package>` paths; for example `addons.keycloak.values` contains chart overrides.
